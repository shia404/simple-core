...
func (_q *MenuQuery) Order(o ...menu2.OrderOption) *MenuQuery {
...
func (_q *MenuQuery) QueryRoles() *RoleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(menu2.Table, menu2.FieldID, selector),
...
func (_q *MenuQuery) First(ctx context.Context) (*Menu, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{menu2.Label}
	}
	return nodes[0], nil
}
...
func (_q *MenuQuery) Only(ctx context.Context) (*Menu, error) {
	...
	case 0:
		return nil, &NotFoundError{menu2.Label}
	default:
		return nil, &NotSingularError{menu2.Label}
	}
}
...
func (_q *MenuQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	...
	case 0:
		err = &NotFoundError{menu2.Label}
	default:
		err = &NotSingularError{menu2.Label}
	}
	return
}
...
func (_q *MenuQuery) IDs(ctx context.Context) (ids []uint64, err error) {
	...
	if err = _q.Select(menu2.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (_q *MenuQuery) Clone() *MenuQuery {
	...
	return &MenuQuery{
		config:       _q.config,
		ctx:          _q.ctx.Clone(),
		order:        append([]menu2.OrderOption{}, _q.order...),
...
func (_q *MenuQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(menu2.RolesTable)
		s.Join(joinT).On(s.C(role.FieldID), joinT.C(menu2.RolesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(menu2.RolesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(menu2.RolesPrimaryKey[1]))
...
func (_q *MenuQuery) loadChildren(ctx context.Context, query *MenuQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *Menu)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(menu2.FieldParentID)
	}
	query.Where(predicate.Menu(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(menu2.ChildrenColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *MenuQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(menu2.Table, menu2.Columns, sqlgraph.NewFieldSpec(menu2.FieldID, field.TypeUint64))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		_spec.Node.Columns = append(_spec.Node.Columns, menu2.FieldID)
		for i := range fields {
			if fields[i] != menu2.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, fields[i])
			}
		}
		if _q.withParent != nil {
			_spec.Node.AddColumnOnce(menu2.FieldParentID)
		}
	}
...
func (_q *MenuQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1 := builder.Table(menu2.Table)
	columns := _q.ctx.Fields
	if len(columns) == 0 {
		columns = menu2.Columns
	}
